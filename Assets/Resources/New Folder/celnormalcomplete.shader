Shader "Custom/CelNormalComplete"
{
	Properties
	{
	...
	SubShader {
		...
		Pass{	
			...
			float4 frag(VertexOutput input) : COLOR {
				...
				float3 diffuseLevel = diffuseColor.rgb * input.color.rgb * atten * _LightColor0.rgb * max(0.0f, normalDotLight);
				...
				float3 diffuseReflection = float3(diffuseColor.rgb) * input.color.rgb *
